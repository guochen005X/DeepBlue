...
def octree_recursive_build(root, db, center, extent, point_indices, leaf_size, min_extent):
    if len(point_indices) == 0: #当子节点没有分到空间点时，该子节点接可以为None,不必去构建Octant
        return None

    if root is None:#初始化8个子节点
    ...
    if len(point_indices) <= leaf_size or extent <= min_extent:
        root.is_leaf = True#根据条件判断是不是叶子节点，如果是叶子节点就返回Octant
    else:
        # 作业4
        # 屏蔽开始,如果不是叶子节点就需要继续分割
        # print(center)
        root.is_leaf = False
        children_indx = [[] for i in range(8)]
        for point_indice in point_indices:
            quadrant = 0
            cur_point = db[point_indice]
            if cur_point[0] > center[0]:#x轴
                quadrant = quadrant | 1
            if cur_point[1] > center[1]:#y轴
                quadrant = quadrant | 2
            if cur_point[2] > center[2]:#z轴
                quadrant = quadrant | 4
            children_indx[quadrant].append(point_indice)

        # for i in range(8):
            # childen_extent = extent * 0.5
            # x_sign = i & 1
            # y_sign = i & 2
            # z_sign = i & 4
            # if x_sign <= 0:
            #     children_center_x = center[0] - childen_extent
            # else:
            #     children_center_x = center[0] + childen_extent
            #
            # if y_sign <= 0:
            #     children_center_y = center[1] - childen_extent
            # else:
            #     children_center_y = center[1] + childen_extent
            #
            # if z_sign <= 0:
            #     children_center_z = center[2] - childen_extent
            # else:
            #     children_center_z = center[2] + childen_extent
        ...
        for i in range(8):
            ...
            children_extent = 0.5 *extent

            childern_center = np.asarray([children_center_x,children_center_y,children_center_z])

            root.children[i] = octree_recursive_build(root.children[i], db, childern_center, children_extent, children_indx[i],leaf_size, min_extent)
...
def overlaps(query: np.ndarray, radius: float, octant:Octant):
    ...
    if np.any(query_offset_abs > max_dist):#球完全在octant外边
        return False
...
def octree_radius_search_fast(root: Octant, db: np.ndarray, result_set: RadiusNNResultSet, query: np.ndarray):
    ...
    if (root.is_leaf and len(root.point_indices) > 0) or contains(query, result_set.worstDist(), root):
...
def octree_construction(db_np, leaf_size, min_extent):
    ...
    db_extent = np.max(db_np_max - db_np_min) * 0.5#正方形边长的一半

    #db_center = np.mean(db_np, axis=0)
    #第一次的中心就不是中间值，而是均值，因此我认为需要修改一下
    db_center = db_np_min + db_extent

    root = None
    root = octree_recursive_build(root, db_np, db_center, db_extent * 2, list(range(N)),
