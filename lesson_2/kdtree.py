...
def kdtree_recursive_build(root, db, point_indices, axis, leaf_size):
    ...
    if len(point_indices) > leaf_size:
        ...
        # 作业1
        # 屏蔽开始
        left_m_indices_sorted = math.ceil(point_indices_sorted.shape[0] / 2) - 1 #获取排序索引数组的一维中间值
        left_m_idx   = point_indices_sorted[left_m_indices_sorted] #根据left_m_indices_sorted得到point_indices_sorted数组的相应数据
        left_m_value = db[left_m_idx,axis]

        right_m_indices_sorted = left_m_indices_sorted + 1
        right_m_idx   = point_indices_sorted[right_m_indices_sorted]
        right_m_value = db[right_m_idx, axis]

        root.value = ( left_m_value + right_m_value ) / 2
        root.left  = kdtree_recursive_build(root.left, db, point_indices_sorted[0: left_m_indices_sorted],
                                            axis_round_robin(axis, db.shape[1]), leaf_size)
        root.right = kdtree_recursive_build(root.right, db, point_indices_sorted[right_m_indices_sorted:],
                                           axis_round_robin(axis, db.shape[1]), leaf_size)
    # else:
    #     root.is_leaf()
#可以用二维数据测试看结果
...
#
# 功能：通过kd树实现knn搜索，即找出最近的k个近邻 根据代码k=8
# 输入：
#     root: kd树
#     db: 原始数据
#     result_set：搜索结果
